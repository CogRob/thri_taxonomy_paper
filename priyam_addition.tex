...
interaction that will be required to complete the task-work. This idea is based
on Norman's model of human-machine interaction \towrite{insert donald norman
reference}. He defines an interaction relationship established between and
artifact and a human by grounding it in 4 factors:
\begin{itemize}
      \item Affordance - Functional definition of the interaction relationship itself.
      \item Signifiers - The properties of the artifact which tell the human
      what affordances can be established.
      \item Feedback - A behavioral signal from the artifact which acknowledges
      the successful establishment of the affordance
      \item Coneptual Model - The sufficient mental model that the agent must
      have of the artifact to support the affordance relationship
\end{itemize}
A good example is one of a person operating an appliation on their smartphone
with a novel user interface. If the interface affords page-turn via pressing of
hot-corners on either sides of the screen, then it should have signifiers making
this affordance clear. In modern UIs, this is done by having a floating
button-like area which bears the sign of an arrow to make it clear that pressing
that area will change the view in the direction of the arrow.
